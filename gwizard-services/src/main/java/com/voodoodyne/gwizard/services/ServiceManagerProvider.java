...
class ServiceManagerProvider implements Provider<ServiceManager> {
	...
	public ServiceManager get() {
		...
		for (Service service: services) {
			for (Service.Listener listener : svcListeners) {
				service.addListener(listener, MoreExecutors.directExecutor());
			}

			addLoggingListenerToService(service);
		}

		for (ServiceManager.Listener listener : mgrListeners) {
			serviceMgr.addListener(listener, MoreExecutors.directExecutor());
		}
